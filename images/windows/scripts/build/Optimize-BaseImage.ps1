...
Write-Host "Disabling hibernation (removes hiberfil.sys)"
...
Get-WinEvent -ListLog * -Force -ErrorAction SilentlyContinue | ForEach-Object {
    try {
        ...
        # Non-fatal: some logs are protected
    }
}
...
Write-Host "Writing zeros to free disk space for thin-provisioning reclamation"
...
Write-Host "Optimize-BaseImage complete"
