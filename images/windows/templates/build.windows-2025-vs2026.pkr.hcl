...
//
// Normal build: packer build -only="windows-2025-vs2026.proxmox-clone.win25-vs2026-runner" .
// Debug build:  packer build -only="windows-2025-vs2026.null.winrm" -var="winrm_host=IP" .
...
build {
  ...
  provisioner "shell-local" {
    only = ["proxmox-clone.win25-vs2026-runner"]
    environment_vars = [
  ...
  provisioner "powershell" {
    environment_vars = ["IMAGE_VERSION=${var.image_version}", "IMAGE_OS=win25-vs2026", "AGENT_TOOLSDIRECTORY=${var.agent_tools_directory}", "IMAGEDATA_FILE=${var.imagedata_file}", "IMAGE_FOLDER=${var.image_folder}", "TEMP_DIR=${var.temp_dir}", "INSTALL_VS_2026=true"]
