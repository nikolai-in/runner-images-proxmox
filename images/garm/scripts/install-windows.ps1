...
Invoke-WebRequest -Uri "https://gitea.com/gitea/act_runner/releases/download/v$ActRunnerVersion/act_runner-v$ActRunnerVersion-windows-amd64.exe" -OutFile "$ActRunnerDir\act_runner.exe"
...
Invoke-WebRequest -Uri "https://github.com/cloudbase/garm/releases/download/$GarmAgentVersion/garm-agent-windows-amd64.exe" -OutFile "$GarmAgentDir\garm-agent.exe"
